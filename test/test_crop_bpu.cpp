...
int main() {
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("example"), "%s", "Test hobotcv_crop: Mat Input, BPU Process");
    auto before_crop = std::chrono::system_clock::now();
    // =============================================
    auto cropmat = hobot_cv::hobotcv_crop(srcmat_nv12, src_height, src_width, dst_height, dst_width, cv::Range(0, dst_height), cv::Range(0, dst_width), hobot_cv::HOBOTCV_BPU);
    // =============================================
    ...
    // 保存结果
    writeImg(cropmat, "./crop_mat_input_bpu_process_1.jpg");
  }

  // 4.crop图像、测试耗时，输入图像为数组格式
  {
    RCLCPP_INFO(rclcpp::get_logger("example"), "%s", "Test hobotcv_crop: Arr Input, BPU Process");
    auto before_crop = std::chrono::system_clock::now();
    // =============================================
    auto imageInfo = hobot_cv::hobotcv_crop(reinterpret_cast<const char *>(srcmat_nv12.data), src_height, src_width, dst_height, dst_width, cv::Range(0, dst_height), cv::Range(0, dst_width), hobot_cv::HOBOTCV_BPU);
    auto after_crop = std::chrono::system_clock::now();
    // =============================================
    auto interval = std::chrono::duration_cast<std::chrono::milliseconds>(after_crop - before_crop).count();
    std::stringstream ss;
    ...
    if (imageInfo != nullptr) {
      ...
      // 保存结果
      writeImg(dst_mat, "./crop_arr_input_bpu_process_1.jpg");
    }
  }
  ...
  // 4.crop图像、测试耗时，输入图像为Mat格式，会缩放再截取
  {
    ...
    auto before_crop = std::chrono::system_clock::now();
    // =============================================
    auto cropmat = hobot_cv::hobotcv_crop(srcmat_nv12, src_height, src_width, dst_height, dst_width, cv::Range(0, dst_height+10), cv::Range(0, dst_width+10), hobot_cv::HOBOTCV_BPU);
    // =============================================
    auto after_crop = std::chrono::system_clock::now();
    auto interval = std::chrono::duration_cast<std::chrono::milliseconds>(after_crop - before_crop).count();
    std::stringstream ss;
    ...
    RCLCPP_INFO(rclcpp::get_logger("example"), "%s", ss_crop.str().c_str());
    // 保存结果
    writeImg(cropmat, "./crop_mat_input_bpu_process_2.jpg");
  }
  ...
  // 4.crop图像、测试耗时，输入图像为数组格式，会缩放再截取
  {
    ...
    auto before_crop = std::chrono::system_clock::now();
    // =============================================
    auto imageInfo = hobot_cv::hobotcv_crop(reinterpret_cast<const char *>(srcmat_nv12.data), src_height, src_width, dst_height, dst_width, cv::Range(0, dst_height+10), cv::Range(0, dst_width+10), hobot_cv::HOBOTCV_BPU);
    auto after_crop = std::chrono::system_clock::now();
    // =============================================
    auto interval = std::chrono::duration_cast<std::chrono::milliseconds>(after_crop - before_crop).count();
    std::stringstream ss;
    ...
    if (imageInfo != nullptr) {
      ...
      cv::Mat dst_mat(imageInfo->height * 3 / 2, imageInfo->width, CV_8UC1, imageInfo->imageAddr);
      // 保存结果
      writeImg(dst_mat, "./crop_arr_input_bpu_process_2.jpg");
    }
  }
